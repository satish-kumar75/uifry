      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 py-10">
        ...
        {footerLinks.map((section, index) => (
          <div className="flex flex-col gap-4 items-start" key={index}>
            <p className="text-3xl font-semibold">{section.title}</p>
            {section.links.map((link, linkIndex) => (
              <p className="font-medium text-sm" key={linkIndex}>
                {link}
              </p>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-4 items-start">
          ...
          <p className="font-medium text-sm">Stay Up To Date</p>
          <div className="flex flex-col gap-2 w-full">
            <input
              type="text"
              className="px-3 py-3 bg-transparent border rounded-md focus:outline-none"
              placeholder="Your email"
            />
            <button className="px-8 py-3 rounded-md w-full bg-black dark:bg-white text-white dark:text-black hover:bg-transparent hover:text-black hover:outline dark:hover:bg-transparent dark:hover:text-white dark:hover:outline transition duration-300 ease-in-out">
              Subscribe
            </button>
          </div>
        </div>
